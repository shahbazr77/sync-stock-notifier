...
class Woo_stock_notifier {
    ...
    if (!class_exists('WooCommerce')) {
        ...
        $isPluginPage = isset($_GET['plugin-name']) && $_GET['plugin-name'] === 'woocommerce-stock-notifications';

        ?>
        <div class="wrap">
        <p class="bread-crumb-opt"><span>Feature Overview >&nbsp;</span><?php echo esc_html__('Synkraft Stock Notifier', 'Synkraft Stock Notifier');?></p>
            <ul class="sub-menu">
                <li><a href="?page=featuresetting.php&plugin-name=woocommerce-stock-notifications&tab=stock-notifier-general" class="<?php echo ($isPluginPage && ($active_tab === 'stock-notifier-general' || empty($active_tab))) ? 'current btn-custom' : 'btn-custom-grey-1'; ?>"><?php echo esc_html__('General Settings', 'woocommerce-stock-notifications');?></a></li>
                <li><a href="?page=featuresetting.php&plugin-name=woocommerce-stock-notifications&tab=users-data" class="<?php echo ($isPluginPage && $active_tab === 'users-data') ? 'current btn-custom' : 'btn-custom-grey-1'; ?>"><?php echo esc_html__('Users Data', 'woocommerce-stock-notifications');?></a></li>
            </ul>
            <?php
                ...
                <table class="wp-list-table widefat fixed striped">
                    ...
                    <tbody>';
                        foreach ($data as $item) {
                            echo '<tr>
                            <th scope="row" class="check-column"><input type="checkbox" name="item[]" value="' . $item->id . '"></th>
